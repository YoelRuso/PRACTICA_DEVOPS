...
def test_remove_expense_reduces_total():
    ...
    # Eliminamos el primer gasto (asumiendo que los IDs de SQLite empiezan en 1)
    service.remove_expense(1)
...
def test_update_expense_partial_fields():
    ...
    # Creamos el gasto inicial
    service.create_expense(
        title="Camiseta", amount=15, description="Ropa", expense_date=date.today()
    )

    # Actualizamos solo el monto (id=1)
    service.update_expense(expense_id=1, amount=18)

    expense = service.list_expenses()[0]
    assert expense.title == "Camiseta"  # Se mantiene
    assert expense.amount == 18  # Cambia
    assert expense.description == "Ropa"  # Se mantiene


def test_total_amount_after_removal():
    service = create_service()

    service.create_expense("Cursos", 30, "", date.today())  # ID 1
    service.create_expense("Internet", 25, "", date.today())  # ID 2
